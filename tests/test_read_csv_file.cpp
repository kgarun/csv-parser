...
TEST_CASE ("col_pos() Test", "[test_col_pos]") {
    int pos = get_col_pos (GetAbsolutePath ("tests/data/real_data/2015_StateDepartment.csv"), "Entity Type");
    REQUIRE (pos == 1);
}
...
TEST_CASE ("Prevent Column Names From Being Overwritten", "[csv_col_names_overwrite]") {
    ...
    for (auto &format_in : formats) {
        ...
        CSVReader reader (GetAbsolutePath ("tests/data/fake_data/ints_comments.csv"), format_in);
...
TEST_CASE ("get_file_info() Test", "[test_file_info]") {
    CSVFileInfo info = get_file_info (GetAbsolutePath ("tests/data/real_data/2009PowerStatus.txt"));
...
TEST_CASE ("Test Read CSV with Header Row", "[read_csv_header]") {
    // Header on first row
    auto path = GetAbsolutePath ("tests/data/real_data/2015_StateDepartment.csv");
...
TEST_CASE ("Test read_row() CSVField - Easy", "[read_row_csvf1]") {
    ...
    CSVReader reader (GetAbsolutePath ("tests/data/fake_data/ints.csv"));
    CSVRow row;
...
TEST_CASE ("Test read_row() CSVField - Power Status", "[read_row_csvf3]") {
    CSVReader reader (GetAbsolutePath ("tests/data/real_data/2009PowerStatus.txt"));
    CSVRow row;

    size_t date = reader.index_of ("ReportDt"), unit = reader.index_of ("Unit"), power = reader.index_of ("Power");
