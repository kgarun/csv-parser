...
const std::string PERSONS_CSV = GetAbsolutePath ("tests/data/mimesis_data/persons.csv");
...
TEST_CASE ("Calculating Statistics from Direct Input", "[read_csv_stat_direct]") {
    ...
    for (int i = 1; i < 101; i++) REQUIRE (reader.get_counts ()[0][std::to_string (i)] == 1);
...
TEST_CASE ("Statistics - Rows of Integers", "[read_csv_stat]") {
    // Header on first row
    auto file = GENERATE (as<std::string>{}, GetAbsolutePath ("tests/data/fake_data/ints.csv"),
                          GetAbsolutePath ("tests/data/fake_data/ints_newline_sep.csv"));

    SECTION ("Compute Statistics") {
        CSVStat reader (file);

        // Expected Results
        std::vector<long double> means = { 50.5, 50.5, 50.5, 50.5, 50.5, 50.5, 50.5, 50.5, 50.5, 50.5 };
